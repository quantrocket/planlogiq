scheduler.attachEvent("onTimelineCreated", function (obj){
	...
	if(obj.render == "tree") {
		...
		scheduler.form_blocks[obj.name]={
			render:function(sns) {
				var _result = "<div class='dhx_section_"+obj.name+"' style='overflow: auto; height: "+sns.height+"px'></div>";
				return _result;
			},
...
scheduler._toggleFolderDisplay = function(key, status, all_sections){ // used for tree view
	...
	scheduler.callEvent("onOptionsLoad",[scheduler.matrix[scheduler._mode]]);
};
