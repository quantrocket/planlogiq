...
class streingabemaskeram extends TPage {
    ...
    public function onInit($param) {
        ...
        if(!isset($this->session['dynamicControlList'])) {
            $this->session['dynamicControlList']=array();
        }
        if(!isset($this->session['RAMRECORD'])) {
            $this->session['RAMRECORD']=array();
        }
                ...
                foreach($this->Perioden AS $key => $value) {
                    ...
                    foreach($this->FunktionsFelder AS $funkID) {
                        ...
                        foreach($myttvalue AS $myttrecord) {
                            ...
                            array_push($this->session['dynamicControlList'],array('class' => "TActiveTextBox", 'id' => $myUniquID, 'OnCallback' => 'onTextChanged','requestStage' => $this->session['requestStage'],'readonly'=>$readonly));
                        }
                    }
                }
                //lets init the label row
                $this->FirstRow = new TActiveTableRow;
                $this->FirstRow->setCssClass('thead');
                $this->resulttable->Rows[]=$this->FirstRow;
                $this->load_header();
                $this->draw_cells();
            }
        //}
    ...
    private function draw_cells() {
        ...
        //hier bauen wir die einzelnen Zeilen
            $this->WorkRow=new TTableRow;
            $this->resulttable->Rows[]=$this->WorkRow;

            if($key<10000) {
                fmod($key,2)==0?$this->WorkRow->setCssClass('inputgrid'):$this->WorkRow->setCssClass('inputgridalternating');
            }

            if(preg_match('/^\d\d\d\d/',$value)) {
                $jahr = $key;
                $monat = $jahr;
            //$this->WorkRow->setBackColor('#cdcdcd');
            }else {
                $jahr = $this->getYearByMonth($key);
                $monat = $key;
            }
            ...
            foreach($this->FunktionsFelder AS $funkID) {
                ...
                foreach($this->session['dynamicControlList'] as $controlDescription) {
                    if($controlDescription['id']==$myUniquID) {
                        ...
                        //$newControl->Text = $controlDescription['Text'];
                        //$this->getControls()->add($newControl);
                        $cell = new TTableCell;
    ...
    private function update_w_wert($jahr,$monat,$local_type,$local_ff,$local_id,$value='0') {
        ...
        $this->session['RAMRECORD'][$myUniquID]=$value;
    //print_r($this->session['RAMRECORD']);
    }
