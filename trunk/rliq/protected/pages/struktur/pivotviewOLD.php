	...
	public function onLoad($param){
        ...
        private function generateGraph($MyArray){
                ...
                foreach($MyArray as $key=>$value){
                    if($dummy <= $this->ParentDimCounter){
                            $dummy++;
                            $xdata[]=$key;
                            array_push($legend,$key);
                    }
					$this->count_Dimension()<=1?'':$counter=0;
					foreach($value As $ax=>$ay){
                        if($counter <= $this->NumberOfElements){
                            $counter++;
							if(!is_array(${'ydata'.$counter})){
								${'ydata'.$counter}=array();
							}
							array_push(${'ydata'.$counter},$ay[0]);
                        }else{
                            $counter=1;
							if(!is_array(${'ydata'.$counter})){
								${'ydata'.$counter}=array();
				...
				for($ii=1;$ii<=$this->NumberOfElements;$ii++){					
        ...
        $arrgarray['numberchildren']=$this->NumberOfElements;
