...
class waren extends TPage{
	...
	private function bindList(){
		...
		$SQL1 .= " LIMIT ".$this->AdresseListe->PageSize * $this->AdresseListe->CurrentPageIndex.",".$this->AdresseListe->PageSize;		
		
		$this->AdresseListe->DataKeyField = 'lstidta_adresse';
		$validate = PFH::checkCountStatement(AdresseRecord::finder()->findBySql($SQL1));
		if($validate){	
			$this->AdresseListe->VirtualItemCount = AdresseRecord::finder()->findBySql($SQL1)->Count();
		}else{
			$this->AdresseListe->VirtualItemCount = 0;
		}
	...
	public function editButtonClicked($sender,$param){
		...
		$warenRecord = WarenRecord::finder()->findByPK($this->edidta_waren->Value);
