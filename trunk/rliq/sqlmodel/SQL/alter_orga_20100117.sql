...
ALTER TABLE `harley2009`.`ta_objekt` ADD COLUMN `obj_nutzflaeche_type` INT(11) NULL DEFAULT NULL COMMENT '1 Wohnzwecke 2 Betrieblich'  AFTER `obj_gbanteile_date` ;
...
ALTER TABLE `harley2009`.`tm_organisation` ADD COLUMN `idta_organisation_art` INT(11) NULL DEFAULT NULL COMMENT 'Eigentum oder Miete'  AFTER `org_specialday_date` , ADD COLUMN `org_auszugsdatum` DATE NULL DEFAULT NULL  AFTER `idta_organisation_art` , ADD COLUMN `org_einzugsdatum` DATE NULL DEFAULT NULL  AFTER `idta_organisation_art` , ADD COLUMN `org_steuerart` VARCHAR(45) NULL DEFAULT NULL COMMENT 'Regelbesteuerung'  AFTER `idta_organisation_art` ;
